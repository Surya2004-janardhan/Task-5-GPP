...
async function main() {
  ...
  if (await test('Create order', async () => {
    const r = await api.post('/api/v1/orders', { amount: 100000, currency: 'INR', receipt: 'test' });
    orderId = r.data.id;
    return r.status === 201 && orderId.startsWith('order_');
  })) passed++;
  total++;

  if (await test('Get order', async () => {
  ...
  console.log('\nPayments:');
  let paymentId;
  if (await test('Create UPI payment', async () => {
    const r = await api.post('/api/v1/payments', { order_id: orderId, method: 'upi', vpa: 'test@upi' });
    paymentId = r.data.id;
    return r.status === 201 && r.data.status === 'pending';
  })) passed++;
  total++;

  if (await test('Get payment', async () => {
  ...
  // 4. Idempotency
  console.log('\nIdempotency:');
  if (await test('Idempotency key returns same payment', async () => {
    const o = await api.post('/api/v1/orders', { amount: 5000, currency: 'INR' });
  ...
  console.log('\nMerchant:');
  if (await test('Get merchant profile', async () => {
    const r = await api.get('/api/v1/merchants/profile');
    return r.status === 200 && r.data.webhook_secret !== undefined;
  })) passed++;
  total++;

  if (await test('Update webhook URL', async () => {
    const r = await api.put('/api/v1/merchants/webhook', { webhook_url: 'http://test.com/webhook' });
    return r.status === 200;
  })) passed++;
  total++;

  if (await test('Regenerate webhook secret', async () => {
  ...
  // 6. Webhook Logs
  console.log('\nWebhook Logs:');
  if (await test('List webhook logs', async () => {
  ...
  // 7. Card Payment
  console.log('\nCard Payment:');
  if (await test('Create card payment', async () => {
    const o = await api.post('/api/v1/orders', { amount: 3000, currency: 'INR' });
  ...
  if (status === 'success') {
    if (await test('Capture payment', async () => {
      const r = await api.post(`/api/v1/payments/${paymentId}/capture`, { amount: 100000 });
      return r.status === 200 && r.data.captured === true;
    })) passed++;
    total++;

    let refundId;
    if (await test('Create partial refund', async () => {
      const r = await api.post(`/api/v1/payments/${paymentId}/refunds`, { amount: 30000, reason: 'Test' });
      refundId = r.data.id;
      return r.status === 201 && r.data.status === 'pending';
    })) passed++;
    total++;
    ...
    if (await test('Refund processed', async () => {
      const r = await api.get(`/api/v1/refunds/${refundId}`);
      return r.data.status === 'processed';
    })) passed++;
    total++;

    if (await test('Exceeding refund rejected', async () => {
      const r = await api.post(`/api/v1/payments/${paymentId}/refunds`, { amount: 100000, reason: 'Fail' });
      return r.status === 400;
    })) passed++;
    total++;
  } else {
