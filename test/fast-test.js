...
async function runFastTests() {
  ...
  console.log(
    "Note: For fastest tests, ensure TEST_MODE=true in docker-compose.yml\n"
  );
  ...
  while (paymentStatus === "pending" && attempts < 20) {
    ...
    process.stdout.write(
      `\r   ⏳ Waiting... ${attempts}s (status: ${paymentStatus})`
    );
  }
  console.log("");
  ...
  if (paymentStatus === "success") {
    ...
    while (refundStatus === "pending" && attempts < 10) {
      ...
      attempts++;
      process.stdout.write(
        `\r   ⏳ Waiting... ${attempts}s (status: ${refundStatus})`
      );
    }
    console.log("");
  ...
  if (webhooks.data.data.length > 0) {
    ...
    console.log(
      `   Latest: ${log.event} - ${log.status} (${log.attempts} attempts)`
    );
  }
