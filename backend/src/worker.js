...
const WEBHOOK_RETRY_INTERVALS_TEST =
...
function getRetryIntervals() {
  return WEBHOOK_RETRY_INTERVALS_TEST
    ? TEST_RETRY_INTERVALS
    : PRODUCTION_RETRY_INTERVALS;
}
  ...
  async (job) => {
    ...
    try {
      ...
      if (refund.payment_status !== "success") {
        console.error(
          `Payment ${refund.payment_id} is not in refundable state`
        );
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (attempts >= 5) {
          ...
          console.log(
            `Webhook ${webhookLog.id} failed permanently after ${attempts} attempts`
          );
        } else {
          // Schedule retry
          ...
          console.log(
            `Webhook ${webhookLog.id} failed, retry scheduled in ${nextRetryDelay}ms`
          );
        }
      }
    } catch (error) {
